...
import {
  ...
  generateRewardCodeForTransaction,
  signOut,
} from "@/app/actions"
import { LogOut, User, DollarSign, Gift, RefreshCw, ArrowRight } from "lucide-react"
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const handleGenerateCode = async (transactionId: string) => {
    const result = await generateRewardCodeForTransaction(transactionId)
    if (result.success) {
      toast.success(result.message)
                ...
                >
                  <div>
                    <p className="font-medium text-gray-800">
                      {reward.profiles?.email || "Unknown User"} redeemed: {reward.description}
                    </p>
                    <p className="text-sm text-gray-600">{new Date(reward.created_at).toLocaleString()}</p>
                    <p className="text-sm text-gray-600">Points: {Math.abs(reward.amount)}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {reward.reward_codes?.code ? (
                      <span className="bg-green-100 text-green-800 text-sm font-semibold px-3 py-1 rounded-full">
                        Code: {reward.reward_codes.code}
                      </span>
                    ) : (
                      <button
                        onClick={() => handleGenerateCode(reward.id)}
